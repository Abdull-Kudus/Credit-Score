...
function getScoreGradient(score: number) {
  if (score >= 800) return { from: "#10b981", to: "#34d399" }
  if (score >= 600) return { from: "#3b82f6", to: "#60a5fa" }
  if (score >= 400) return { from: "#f59e0b", to: "#fbbf24" }
...
function ScoreGauge({ score }: { score: number }) {
  ...
  const scoreDash = (score / 1000) * HALF_CIRC
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <svg width={GAUGE_W} height={GAUGE_H} viewBox={`0 0 ${GAUGE_W} ${GAUGE_H}`} className="overflow-visible">
        ...
        <text x={ARC_CX} y={ARC_CY - ARC_R - 6} textAnchor="middle" className="fill-muted-foreground/40 text-[9px] tabular-nums">500</text>
        <text x={ARC_CX + ARC_R + 2} y={ARC_CY + 14} textAnchor="middle" className="fill-muted-foreground/40 text-[9px] tabular-nums">1000</text>
      </svg>
...
export function HealthScore({ overall, trend, trendDelta, factors }: HealthScoreProps) {
